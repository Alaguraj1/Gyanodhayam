...
export const singleSlugData = async (slug) => {
  try {
    const res = await axios.get(
      `https://file.gyanodhayam.org/wp-json/wp/v2/pages?slug=${slug}`
    );
    return res.data;
  } catch (error) {
...
export const gallerySlugData = async (slug) => {
  try {
    const res = await axios.get(
      `https://yourdomine.com/wp-json/wp/v2/pages/?slug=${slug}`
    );
...
export const accordiantSlugData = async (slug) => {
  try {
    const res = await axios.get(
      `https://yourdomine.com/wp-json/wp/v2/pages/?slug=${"your-slug"}`
    );
    const html = res.data[0]?.content?.rendered;
    var parser = new DOMParser();
    var doc = parser.parseFromString(html, "text/html");
